(() => {
  "use strict";
  // DPRO GREEN PRODUCT EVERGREEN OWNER UI R1.9.3 / active site-area filter / 2026-09-22
  ...
  async function assetMoveForm(asset, type) {
    ...
    const updateAreas = async () => { const siteId = $("#move-site").value; if (!siteId) { $("#move-area").innerHTML = '<option value="">設置場所を選択</option>'; return; } const result = await Green.api(`/api/admin/sites/${siteId}`); $("#move-area").innerHTML = optionList((result.data.areas || []).filter((i)=>i.is_active !== false),"id",(i)=>i.area_name,asset.current_site_area_id,"設置場所を選択"); };
